...
%% load data
load('./data/JIDcompiled_and_listed/JIDcompiledHourly_sel.mat')
load('./data/ape_padded_and_non_padded_v5.mat')
load('./data/perferred_ranks_long_v5.mat')
load('./data/ForCluster.mat', 'Period')
load('./data/to_plot_fig3_long_v5.mat')
load('./data/to_plot_fig4_long_v5.mat')

%% initialization
n_clusters = size(binary_presence, 2);
valid_long = cellfun(@(x) size(x, 1) == 397, ape_jids);
b_beg = 190;
b_end = 390;
concat_all_px = cell(1, n_clusters);
n_subs_full = 401;

% this is due to python loading a MATLAB tensor of size X, Y, Z in the 
% order Z, Y, X
re_padded_clu = reshape(padded_clu, n_subs_full, 397, 50, 50);
p_re_padded_clu = permute(re_padded_clu, [1,2,4,3]);
padded_clu = reshape(p_re_padded_clu, n_subs_full, 397, 2500);
...
for scale_k = 1:n_clusters
    ...
    % select only the subjetcs with > 180 days
    ...
    ori_idx = sub_id(valid_long);
    k_idx = 1:n_subs_full;
    valid_idx = k_idx(valid_long);
    ...
    for IDXA = 1:length(sub_idx_valid_for_scale)
            ...
            ori_id_a = sub_idx_valid_for_scale(IDXA);
...
booted = zeros(n_clusters, n_boot);
